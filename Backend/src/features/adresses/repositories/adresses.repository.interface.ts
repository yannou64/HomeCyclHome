...
export interface IAdressesRepository {
    ...
    findByIdAndUser(id: string, utilisateurId: string): Promise<AdresseDto | null>;

    // Cherche une adresse physique par google_place_id (déduplication à la création)
    findAdresseByGooglePlaceId(googlePlaceId: string): Promise<{ id: string } | null>;

    // Crée l'adresse physique si elle n'existe pas, puis crée la liaison PeutSeSituer
    create(utilisateurId: string, data: CreateAdresseInput): Promise<AdresseDto>;

    // Modifie uniquement titre_description (adresse physique immuable)
    updateMetadata(id: string, data: Pick<UpdateAdresseInput, 'titreDescription'>): Promise<AdresseDto>;
